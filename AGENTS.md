...
1. **本质**
   - Reader RS 是 PDF 文档文本搜索与提取工具：Rust 编写的 CLI，对本地 PDF 做按页文本提取与关键词/正则搜索。

2. **边界**
   - 只做本地 PDF 的文本层：提取与搜索。首版不做渲染、不做编辑、不做 OCR（扫描件检出后提示，不识别）。
   - CLI 是唯一交互面；纯 Rust 依赖，不外挂 pdfium 等二进制运行时。
   - Windows 优先验证；依赖均跨平台，不主动破坏其它平台。

3. **管理对象**
   - 本地 PDF 文件（只读）。
   - CLI 用户：`reader` 命令（等价缩写 `rr`，同一二进制两个名字）。

4. **方案索引**
   - 定位：`docs\references\R001-项目定位-PDF文本搜索与提取工具.md`
   - 首期方案：`docs\proven\P0001-PDF文本搜索与提取CLI最小闭环.md`
...
> 需求意图与操作方法的映射。显示名 Reader RS；仓库 `reader_rs`；CLI 二进制 `reader`（缩写 `rr`）。

- **搜文本**：`reader search <文件> <关键词>`（`--regex` 正则、`-i` 忽略大小写、`-C N` 上下文、`--pages 1-3,5` 限页；命中退出 0、无命中退出 1、出错退出 2）
- **提文本**：`reader extract <文件>`（`--pages` 限页、`-o` 写文件；按页输出 `== page N ==` 分节）
