...
- 遇缺陷当场一键反馈：`reader issue new "<标题>" --kind bug --acceptance "<验收判据>"`（仓级公共账本 ledger.ohmygh.com，统一 crate ledger-client 签名道，REQ-060/061 承接 REQ-063；CLI 只增不关不删，关闭走 omc 工位）；读面 `issue list` / `issue show`，产物沉淀 `artifact publish`
