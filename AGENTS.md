...
- 遇缺陷当场一键反馈：`reader issue new "<标题>" --kind bug --acceptance "<验收判据>"`（仓级公共账本 ledger.ohmygh.com，Ed25519 五头签名，REQ-060 承接 REQ-063；关单 `issue close` 须 result 引 digest）；读面 `issue list` / `issue show`，产物沉淀 `artifact publish`
