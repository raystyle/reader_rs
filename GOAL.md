...
- 当前目标（已达成）：P0010 无标题分节加 S005 TOON 验证。下版候选：阶段 3 余项（MCP、token 计数、pager、completions、批量目录）与发布面余项（musl 静态、crates.io/brew/scoop 分发）及「有标题单节超长再分片」，待用户点名立项；TOON 已销案（S005）。
