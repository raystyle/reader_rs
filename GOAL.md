...
- **日期**：2026-08-31。
- **起点**：PDF 与 EPUB 双格式闭环后，用户要在 macOS 与 Linux 下接管开发与测试；仓库需三平台开箱可用（CI 门禁、换行符、文档命令形态）。

## 锚点

> 当前锚定的目标 + 推进时间线。

- **锚定的目标**：mac 与 Linux 接管开发与跨平台兼容（方案 P0004）。
...
- 当前目标（已达成）：mac 与 Linux 接管开发与跨平台兼容（P0004）。CI 首跑三系统绿。下一刀候选见 ROADMAP 阶段 2/3，待用户点名立项。
