...
- 当前目标（已达成）：P0011 加 P0012 达成并经 Linux 实机验收对账闭环（R004 回填）；P0013 musl 本地去险完成，验收转 v0.2.1 tag 首跑（五 job 绿 9 资产齐），**v0.2.1 tag 可发**；M007（SIGPIPE）验收中发现并当轮修复。下版候选：阶段 3 余项（MCP、token 计数、pager、completions）与发布面余项（crates.io/brew/scoop 分发），待用户点名立项。
