...
- **日期**：2026-09-03。
- **起点**：用户指示对项目自省，学习 pve-harness 的 PRD 文档；自省发现三原语缺需求层（需求散落对话与归档、无澄清记录、拒绝项无防复问、追溯断头）。裁定需求层独立为第四原语（D27），分工经用户逐条定：PRD 需求清单、GOAL 目标与达成标准、PLAN 规划、TODO 进度。

## 锚点

> 当前锚定的目标 + 推进时间线。

- **锚定的目标**：D27 PRD 四原语落地（已达成：PRD.md 需求清单建立，AGENTS / INDEX / G003 同步）；上一目标 P0018 OCR 换引擎已收官，v0.3.0 已发布。
...
- 当前目标（已达成）：D27 PRD 四原语落地，见 `PRD.md` 与 AGENTS 二节。
