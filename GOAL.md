...
- **锚定的目标**：D28 / D29 / D30 测试体系与 PoC 目录落地（已达成：R006 六层规范、tests\ab\ A/B 层首跑 tiny vs small、poc\ 目录约定与 S006 迁移）；上一目标 D27 PRD 四原语已收官。
...
| 日期 | 进展 |
| --- | --- |
| 2026-09-03 | **D28 / D29 / D30 达成**：R006 六层测试规范（单元/集成/冒烟/回归/验收/A/B）落盘；tests\ab\ A/B 层建成（manifest 对象资源、独立来源检查点、ab_run.py 跑批器）并首跑 tiny vs small（真样本 5/5 对 5/5 与 37 对 51 行、1.99s 对 5.13s/页；合成样本 tiny 1/5 对 small 4/5）；`READER_OCR_MODEL_SIZE` 档位开关进 src\ocr.rs；poc\ 目录约定落地（G002 八节），S006 PoC 源码迁入 |
| 2026-09-03 | **D27 达成**：学习 pve-harness PRD 自省落地：`PRD.md` 需求清单建立（D01 至 D22 追溯登记 [推断]、D15 TOON 拒绝防复问、D23 至 D26 候选待澄清、D27 本项）；AGENTS 工作节奏改四原语加追问链规则、候选方向挪入 PRD；INDEX 与 G003 登记同步 |
...
- 当前目标（已达成）：D28 / D29 / D30 测试体系与 PoC 目录，见 R006、tests\ab\、`poc\README.md`。
...
| 日期 | 目标 | 结果 |
| --- | --- | --- |
| 2026-09-03 | 测试体系六层规范与 A/B 层、PoC 目录（D28 / D29 / D30） | 达成：R006 落盘；tests\ab\ 建成并首跑 tiny vs small（报告 tests\ab\reports\2026-09-03-tiny-vs-small.md）；OCR 模型档位 env 开关；poc\ 约定与 S006 迁移 |
| 2026-09-03 | PRD 四原语落地（D27） | 达成：PRD.md 需求清单建立（27 条：追溯 22、拒绝 1、候选 4）；AGENTS / INDEX / G003 同步四原语；分工定义经用户逐条裁定 |
