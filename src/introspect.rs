...
pub fn llms_text() -> String {
    ...
    format!(
        "\
reader v{v} — Agent 原生文档阅读、搜索和提取工具（PDF 按页 / EPUB 按章 spine 序，只读文本层；缩写 rr 同入口）
...
输出 text: 命中行 单元:行号:文本；上下文 单元-行号-文本；extract 节头 == page N == 或 == chapter N ==
...
---
name: reader
description: Agent 原生文档阅读、搜索和提取工具。从本地 PDF / EPUB 读文本层——按页或章读、按词或正则搜、按页或章取。输出稳定可解析，grep 语义退出码。
---

# Reader

Rust 单二进制 CLI（v{v}；命令 `reader`，缩写 `rr` 同入口）。只读本地文档文本层：PDF 按页、EPUB 按章（spine 阅读序）。无交互、无守护进程；机器可读优先，错误走 stderr。

## 何时使用

- 在本地 PDF / EPUB 中定位关键词或正则命中（行式 `单元:行号:文本`，直接可解析）。
- 把文档文本层喂给 LLM 上下文：大文档用 `--offset` / `--limit` 分页，按 meta 的 `next_offset` 与 `cta` 链式推进。
- 要结构化结果：`--format json` 包膜，`--filter` 点路径裁剪只取所需字段。

不适用：扫描件 OCR（只检出并以 needs_ocr 提示，不识别）、渲染、编辑、PDF / EPUB 以外格式。
...
- search 命中行 `单元:行号:文本`；上下文行 `单元-行号-文本`。PDF 单元是页，EPUB 单元是章。
- extract 按单元分节，节头 `== page N ==` 或 `== chapter N ==`；PDF 行为 markdown 形态（标题、链接、表格语法）。
- 文本层不可靠页（扫描件、编码问题）：extract 在节头后出 `[needs_ocr: 原因]` 提示行；search 走 stderr 警示，stdout 不混入。
