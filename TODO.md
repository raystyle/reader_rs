# TODO：当前目标任务进度清单

> 角色：**当前目标的任务进度清单**。当前目标完成后，过程与经验回填到 `docs\proven\` 对应方案，并起新清单；当天做的事只记 `docs\diary\`。

## 当前目标

用户点名三项按序推进（登记日 2026-09-01）。**P0011 与 P0012 已达成并经 Linux 实机验收（R004）对账闭环**；验收中发现并当轮修复 M007（SIGPIPE 管道 panic，15 单元 + 44 集成全绿，CI 三系统绿）；**P0013 musl 本地预建过（静态件成、zstd-sys 实证），验收挂 v0.2.1 tag 首跑**（五 job 全绿 9 资产齐），v0.2.1 tag 可发，待用户确认发版。**mac 接管开发已移交（R005）**：仓已 pull、样本与工具面就位，待 mac 侧跑验收清单。

## 任务进度清单

| 任务项 | 进度 | 说明 | 日期 |
| --- | --- | --- | --- |
| 立项 P0011-P0013 | 已完成 | 三方案、三原语、INDEX 登记 | 2026-09-01 |
| P0011 超长节再分片 | 已完成 | `to_unit_bodies` 逐节判定；混排夹具绿；feat 提交 23ba52b | 2026-09-01 |
| P0012 批量目录搜索 | 已完成 | `src\batch.rs`、`is_supported`、批量四态用例；真样本目录冒烟过 | 2026-09-01 |
| P0013 musl 资产 | 进行中 | 矩阵与 musl-tools 步骤已提交（081ab9e）；Linux 本地预建过（静态件成）；验收 = v0.2.1 tag 首跑五 job 绿 | 2026-09-01 |
| Linux 实机验收（R004） | 已完成 | 门禁与文档门禁全绿、真样本五路过；发现并修 M007；验收记录已回填 R004 | 2026-09-01 |
| M007 SIGPIPE 修复 | 已完成 | main 恢复 SIGPIPE 默认处置（unix）、libc 直接依赖、回归测试；141 静默双验（debug/musl） | 2026-09-01 |
| mac 接管移交（R005） | 已完成 | 清单落、仓 pull 到 154df2b、样本 docx SCP 就位、工具面实测；mac 侧验收待跑 | 2026-09-01 |
| 收官登记 | 已完成 | CHANGELOG、introspect/SKILL 再生成、README、diary、GOAL/INDEX、门禁全绿 | 2026-09-01 |
