...
| 6 | 分发自治：模型镜像源链与 self update 镜像通道、ocr 管理组（D42） | 已完成 |
...
图片文件进格式面（D43）、图片本体导出与元数据对齐加一键完整提取（D47 figures/export,S010 定界）、测试面大扩（D44 官方语料全量 + D46 研究资料基线）、版本分支模型（D45 dev/main）。dev/v0.6.0 全平台验收绿,待封版。
