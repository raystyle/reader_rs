...
> 迁移注记（2026-09-16）：方案职责由 REQ 的 Criteria 加 ADR 承接（dev-evo 体系，映射见 `docs\README.md`）；本文件为历史档案，当前目标 D43 至 D47 已随 v0.6.0 收官（见 GOAL 时间线与 ROADMAP 阶段 7）。

## 当前目标:D43 图片文件分析 加 D44 全格式冒烟覆盖

> 随行 D42 收尾与同分支扩项见下节立项说明。

> 2026-09-04 立项。依据:PRD D43 / D44;S009 研究(零新依赖实证);用户四裁(格式集八种、--ocr opt-in、首帧、冒烟主干全补)与官方语料裁定(anydoc fixtures 入仓)。同分支随行扩项(各见 PRD):D45 分支模型、D46 研究资料语料(materials)、D47 图片本体(figures v1.1 内嵌直抽加 export 一键提取,S010)。D42 主体已收官(v0.5.0),收尾两件随本目标携带:M017 补丁(下载器自建父目录)随下版发布;P0019 归档等 ISSUE #1 大陆侧第三级回退演练回执。
