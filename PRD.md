...
| D46 | 本地电子书库 E:\ebook 作为主性能与质量测试面（114 件：103 pdf 加 10 epub，约 4GB 真样本） | 已采纳 | 第 1 轮点名（2026-09-04）；形态裁定：外部真样本不入仓，manifest 钉 sha256 身份，质量基线（status / units / needs_ocr 确定性字段）gated 回归（`tests\ebook.rs`，盘缺失整体跳过 CI 免跑），性能口径归报告不进断言（`.tools\ebook-corpus.py --perf`） | 语料构成实证 2026-09-04 扫盘；与 anydoc 官方语料（D44 格式正确性快扫）互补：ebook 承载 pdf-inspector 真书行为与体量性能 |
