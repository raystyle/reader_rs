...
| D47 | 图片本体自动提取与文本元数据对齐：reader 自动提取文本和图片，提取的元数据对齐（figures 面）；图表数据还原 PoC | 已采纳 | 第 1 轮三裁（2026-09-04）：T1 figures 立项落地、T2 几何配对 PoC 先行、T3 模型路线处置问；第 2 轮定界「只负责提取存储图片，和文本元数据对齐」:T3 模型理解已拒绝（防复问：理解归调用方 Agent 多模态侧，reader 不载模型不破纯 Rust 边界） | 研究 S010（三层路线、仓内基石源码实证：ppocr OcrLine.polygon、anydoc Asset、hayro 渲染；生态双通道：PP-Chart2Table 破边界、Rust 无数字化 crate）；PoC 落 `poc\s010-chart-geometry\` |
