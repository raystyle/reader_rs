...
> 迁移注记（2026-09-16）：新需求改走 `docs\requirements\` REQ（D 号即 REQ 号口径：D01 至 D47 历史留档；活跃队列 D23 至 D26 与 D36 至 D39 已转 REQ-023 至 REQ-026 与 REQ-036 至 REQ-039，迁移本身立 REQ-048，此后自 REQ-049 接编）；本清单为历史档案，状态与裁定不再更新。
...
| D46 | 本地语料库作为主性能与质量测试面（第 1 轮 E:\ebook 172 件 2.26GB；第 2 轮用户裁定弃 ebook 改 E:\研究资料 575 件 5.4GB，551 pdf 加 pptx/epub/docx/ppt/xlsx/png/md） | 已采纳 | 第 1 轮点名（2026-09-04）落 ebook 全套后，第 2 轮裁定弃用换研究资料库（机制平移改名 materials，E:\ebook manifest 与基线废弃）；形态裁定不变：外部真样本不入仓，manifest 钉 sha256 身份，质量基线（status / units / needs_ocr 确定性字段）gated 回归（`tests\materials.rs`，盘缺失整体跳过 CI 免跑），性能口径归报告不进断言（`.tools\materials-corpus.py --perf`） | 语料构成实证 2026-09-04 两轮扫盘；与 anydoc 官方语料（D44 格式正确性快扫）互补：materials 承载 pdf-inspector 真实研究文档行为与体量性能 |
| D47 | 图片本体自动提取与文本元数据对齐：reader 自动提取文本和图片，提取的元数据对齐（figures 面）；图表数据还原 PoC | 已采纳 | 第 1 轮三裁（2026-09-04）：T1 figures 立项落地、T2 几何配对 PoC 先行、T3 模型路线处置问；第 2 轮定界「只负责提取存储图片，和文本元数据对齐」:T3 模型理解已拒绝（防复问：理解归调用方 Agent 多模态侧，reader 不载模型不破纯 Rust 边界）；第 3 轮点名（2026-09-04）：一键完整提取 `export`（文本+图片+元数据落一目录或指定页，PDF 与 EPUB 都要支持）加导出目录二次复用搜索（pages/ 逐单元 markdown）；第 4 轮验收反馈补 PDF 内嵌位图直抽（论文插图原字节，DCT jpg / Flate png；扫描页才整页渲染） | 研究 S010（三层路线、仓内基石源码实证：ppocr OcrLine.polygon、anydoc Asset、hayro 渲染；生态双通道：PP-Chart2Table 破边界、Rust 无数字化 crate）；PoC 落 `poc\s010-chart-geometry\`；真机实证：certmitm 论文 28 件内嵌 JPEG 逐页抽、扫描书页回退渲染、EPUB 347 节加 26 图 export、二次搜索页锚命中 |
