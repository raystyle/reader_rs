...
> 迁移注记（2026-09-16）：dev-evo 迁移归档件。本文件为旧 AGENTS.md 全文快照（三节合同：项目定位、工作规则、文档索引），协作规则唯一权威已改由根 `AGENTS.md` 五节合同承载；四原语协作规则见下文「每轮对话」条目。
> 清退注记（2026-09-17）：本文引用的四原语与 proven 与 mistakes 档案已清退（ADR-0006），引用仅作历史快照，不再指向仓内文件。
