...
- **2026-09-16，lan-mac（arm64，仓切 main 到 d038c59）**：门禁三件全绿（74 集成含 sigpipe、doctest 4 例等 11 组 result ok）。坑一枚：仓分支跟踪残留已删的 dev/v0.6.0，`git pull --ff-only` 静默停在 ad09c3e 旧码，fetch 加 checkout main 后复跑（详见 diary 2026-09-16 第 11 条）；实机验收进门禁前必核 `git log -1` 与 Cargo.toml version [实证: ssh 实跑各命令退出码 0]。
