...
- **2026-09-16，lan-ubuntu（x86_64 NUC 十六核，rustup stable 1.98.1 新装，深克隆 d038c59）**：门禁三件全绿（fmt 0 diff、clippy 0 告警、test 11 组 result ok：41 单元 + 74 集成含 sigpipe + 64 语料快照 + doctest 4 例等）；四平台实机测试协议（用户宣言 2026-09-16）的 Linux NUC 首轮 [实证: ssh 实跑各命令退出码 0]。lan-linux（server）同本清单适用，按需入环。
